...
void diffuse(int n, int b, float *dens, float *dens0, float rate, float dt) {
...
void advect(int n, int b, float *dens, float *dens0, float *u, float *v, float dt) {
	float dt0 = dt * n;
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
...
void project(int n, int h, float *u, float *v, float *p, float *div) {
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= n; j++) {
...
void velocStep(int n, int h, float *u, float *v, float *u0, float *v0, float visc, float dt) {
	...
	project(n, h, u, v, u0, v0);
	SWAP(u, u0); SWAP(v, v0);
	advect(n, 1, u, u0, u0, v0, dt); advect(n, 2, v, v0, u0, v0, dt);
	project(n, h, u, v, u0, v0);
}

void densStep(int n, float *dens, float *dens0, float *u, float *v, float diffRate, float dt) {
	add_source(n, dens, dens0, dt);
	SWAP(dens, dens0);
	diffuse(n, 0, dens, dens0, diffRate, dt);
	SWAP(dens, dens0);
